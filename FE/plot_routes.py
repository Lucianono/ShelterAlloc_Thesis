        ...
        for _, shelter in shelters_df.iterrows():
            try:
                ...
                if progress_callback:
                    progress_callback(f"Route from {community['Name']} to {shelter['Name']} completed: {route_distance / 1000:.2f} km")
                print(f"Route from {community['Name']} to {shelter['Name']}: {route_distance / 1000:.2f} km")

            except nx.NetworkXNoPath:
                print(f"No path found from {community['Name']} to {shelter['Name']}.")
                continue
