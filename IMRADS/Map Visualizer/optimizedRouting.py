...
def plot_optimized_routes(allocation_df, comm_dict, shel_dict, map_name="optimized-routes-map.html"):
    ...
    allocation_df['Latitude_Shel'] = allocation_df['Shelter Initial'].map(lambda x: shel_dict.get(x, {}).get('x'))
    allocation_df['Longitude_Shel'] = allocation_df['Shelter Initial'].map(lambda x: shel_dict.get(x, {}).get('y'))
    allocation_df['Latitude_Shel_Trans'] = allocation_df['Shelter Transfer'].map(lambda x: shel_dict.get(x, {}).get('x'))
    allocation_df['Longitude_Shel_Trans'] = allocation_df['Shelter Transfer'].map(lambda x: shel_dict.get(x, {}).get('y'))

    # Remove any rows with missing coordinates
    allocation_df.dropna(subset=['Latitude_Comm', 'Longitude_Comm', 'Latitude_Shel', 'Longitude_Shel', 'Latitude_Shel_Trans', 'Longitude_Shel_Trans'], inplace=True)
    ...
    for _, row in allocation_df.iterrows():
        ...
        folium.Marker([row['Latitude_Shel'], row['Longitude_Shel']], popup=f"Shelter: {row['Shelter Initial']}", icon=folium.Icon(color='blue')).add_to(m)
        folium.Marker([row['Latitude_Shel_Trans'], row['Longitude_Shel_Trans']], popup=f"Shelter: {row['Shelter Transfer']}", icon=folium.Icon(color='blue')).add_to(m)
    
    #folium.Marker([14.896103367692303 , 120.77693709230768 ], icon=folium.Icon(color='orange')).add_to(m)

    used_shelters = list(set(allocation_df['Shelter Initial']))
...
# Draw the route path (A* path)
    ...
    for idx, row in allocation_df.iterrows():
        try:
            ...
            trans_location = (row['Latitude_Shel_Trans'], row['Longitude_Shel_Trans'])


            AntPath([start_location,end_location], color="red", weight=6, delay=1000).add_to(straight_layer)
            AntPath([end_location,trans_location], color="purple", weight=6, delay=1000).add_to(straight_layer)


        except nx.NetworkXNoPath:
            print(f"No path found from {row['Community']} to {row['Shelter Initial']}.")
            continue
        except Exception as e:
            print(f"Error processing route from {row['Community']} to {row['Shelter Initial']}: {e}")
            continue
...
def run_optimization(communities_file='modelCommData.xlsx', shelters_file='modelShelData.xlsx', allocation_file='BST_alloc.xlsx'):
    ...
    #allocation_df = pd.read_excel(allocation_path, usecols=["Community", "Shelter Initial"])
    allocation_df = pd.read_excel(allocation_path, usecols=["Community", "Shelter Initial", "Shelter Transfer"])
