...
def plot_optimized_routes(allocation_df, comm_dict, shel_dict, map_name="optimized-routes-map.html"):
    ...
    allocation_df['xDegrees_Comm'] = allocation_df['Community Name'].map(lambda x: comm_dict.get(x, {}).get('x'))
    allocation_df['yDegrees_Comm'] = allocation_df['Community Name'].map(lambda x: comm_dict.get(x, {}).get('y'))
    ...
    for _, row in allocation_df.iterrows():
        folium.Marker([row['xDegrees_Comm'], row['yDegrees_Comm']], popup=f"Community: {row['Community Name']}", icon=folium.Icon(color='green')).add_to(m)
...
# Draw the route path (A* path)
    ...
    for idx, row in allocation_df.iterrows():
        try:
            ...
            print(f"No path found from {row['Community Name']} to {row['Shelter Assigned']}.")
            continue
        except Exception as e:
            print(f"Error processing route from {row['Community Name']} to {row['Shelter Assigned']}: {e}")
            continue
...
def run_optimization(communities_file='commData.xlsx', shelters_file='shelData.xlsx', allocation_file='allocation_results.xlsx'):
    ...
    allocation_df = pd.read_excel(allocation_file, usecols=["Community Name", "Shelter Assigned"])
