# Bilevel Sequential Transfer model
...
class BSTModel:
    ...
    def __init__(self):
        ...
        self.mutation_iteration = 3

        self.weight_dist = 0.5
    ...
    def fitness(self, allocation):
        ...
        # the actual model
        objective_value = self.weight_dist * total_distance + self.weight_cost * total_cost
    ...
    def check_transferred_capacity(self, allocation):
        ...
        for community in Community:
            ...
            if shelter_name:
                # add to used_area based on population
                required_area = community["population"] * community["portiontransfer"] * self.area_per_individual
                used_area[shelter_name] += required_area
    ...
    def check_max_lvl2_shelters(self, allocation):
        lvl2_shelters_ctr = sum(1 for level in allocation["shelterlvl"].values() if level == 2)
        penalty = 0

        if lvl2_shelters_ctr > self.max_lvl2_shelters:
            # print("max lvl2 shelters constraint failed")
            penalty += lvl2_shelters_ctr - self.max_lvl2_shelters
       
        return penalty

    def check_initial_lvl1_shelters(self, allocation):
        initial_shelters = set(allocation['initial'].values())
        penalty = 0
    ...
    def check_transfer_lvl2_shelters(self, allocation):
    ...
    def getPenaltySum(self, allocation):
         return (self.check_initial_capacity(allocation)**2 + 
                self.check_transferred_capacity(allocation)**2 + 
                self.check_max_distance(allocation)**2 + 
                self.check_max_shelters(allocation)**2 + 
                self.check_max_lvl2_shelters(allocation)**2 +
                self.check_initial_lvl1_shelters(allocation)**2 +
                self.check_transfer_lvl2_shelters(allocation)**2)
    ...
    def feasibilityCheck(self):
        ...
        for community in Community:
            if not (
                any(shelter["area1"] * self.area_per_individual >= community["population"] for shelter in Shelters) or
                any(shelter["area2"] * self.area_per_individual >= community["population"] for shelter in Shelters)
            ):
                failing_communities.append(community["name"])
    ...
    def start_model(self):
        ...
        for _ in range(self.num_solutions):
            solution = self.spawn()
            fitnessVal = self.fitness(solution)
        ...
        for generation in range(self.num_generations):
            ...
            for i in range(self.num_solutions):
                ...
                offspring = self.generate_offspring(mother, father)
            ...
            best_solutions = sorted(best_solutions, key=lambda x: x[0])[:self.num_solutions] 

            if (generation+1) % 100 == 0 :
                print(best_solutions[0])
                print(f"=== Gen {generation+1} best solution ===")

            prev_best_solution = self.fitness(self.solutions[0])

            # replace old population
            self.solutions = [sol[1] for sol in best_solutions]

            new_best_solution = self.fitness(self.solutions[0])
            if len(self.obj_value_updated_arr) == 0:
                self.obj_value_updated_arr.append(prev_best_solution)
            ...
            if(prev_best_solution != new_best_solution):
                ...
                self.obj_value_updated_arr.append(new_best_solution)
        ...
        return {
            ...
            "runtime": f"{minutes}m {seconds:.2f}s",
