...
"}")
    ...
    def insert_new_student(self):
        ...
        # Create list for student info
            self.new_student = [
            self.name_lineEdit.text(),
            student_id,
            gender,
            self.grade_comboBox.currentText(),
            birthday,
        ...
        # Insert query with parameterized placeholders
            insert_student_query = """ 
                INSERT INTO student_table (name, student_id, gender, class, birthday, address, phone_number, email)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """
            cursor.execute(insert_student_query, self.new_student)

        # Show message and commit changes
    ...
    def generate_studentId(self, gender):
        ...
        while True:
            id_start_value = '24' + ('/U/M' if gender == "Male" else '/U/F')
            random_value = self.generate_randomNumber()
            student_id = id_start_value + random_value

        # check if id exists
            cursor.execute(f"SELECT student_id FROM student_table WHERE student_id = %s", (student_id,))
    ...
    def show_inserted_message(self):
        ...
        msg_box.setText(self.name_lineEdit.text() + " was added successfully")
        msg_box.exec_()
